...
public:
    ...
    // Método para actualizar una clave existente
    void actualizar(int clave_antigua, int clave_nueva) {

        Node* curr = root;

        // Buscar la hoja donde se encuentra la clave
        while (!curr->leaf) {

            int i = 0;

            while (i < curr->keys.size()) {
                if (clave_antigua < curr->keys[i]) {
                    break;
                }
                i++;
            }
        ...
        for (int i = 0; i < curr->keys.size(); i++) {
            ...
            if (curr->keys[i] == clave_antigua) {

                curr->keys[i] = clave_nueva;

                std::sort(curr->keys.begin(),
                        curr->keys.end());
...
int main() {
    ...
    std::cout << "\n--- Actualizando la clave 5 por 15 ---" << std::endl;
    tree.actualizar(5, 15);

    // Mostrar árbol después de actualizar
    tree.printTree();
